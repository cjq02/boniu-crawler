...
from crawler import BaseCrawler, CrawlResult
from config import get_settings
from utils import clean_text, extract_links_from_html, extract_json_from_html
...
class RequestsCrawler(BaseCrawler):
    ...
    def crawl_url(self, url: str, **kwargs) -> CrawlResult:
        ...
        try:
            ...
            self.logger.crawl_success(url, 1)
    ...
    def batch_crawl(self, urls: List[str], **kwargs) -> List[CrawlResult]:
        ...
        self.logger.info(f"批量爬取完成: 成功 {len([r for r in results if not r.error])}, 失败 {len([r for r in results if r.error])}")
        return results
...
class AsyncRequestsCrawler(BaseCrawler):
    """异步requests爬虫类"""
    
    def __init__(self, name: str = "async_requests_crawler"):
        super().__init__(name)
        self.ua = UserAgent()
    ...
    async def crawl_url(self, url: str, **kwargs) -> CrawlResult:
        ...
        # 应用反爬虫策略
        await self.apply_anti_crawler_strategy()
        ...
        try:
            timeout = aiohttp.ClientTimeout(total=kwargs.get('timeout', self.settings.crawler.timeout))
            ...
            async with aiohttp.ClientSession(timeout=timeout) as session:
                async with session.get(url, headers=headers) as response:
                    ...
                    # 解析数据
                    parsed_data = self.parse_data(data, url)
    ...
    async def batch_crawl(self, urls: List[str], **kwargs) -> List[CrawlResult]:
        """
        异步批量爬取URLs
        ...
        self.stats.total = len(urls)
        self.logger.info(f"开始异步批量爬取 {len(urls)} 个URL")
        
        # 创建限流器
        throttler = asyncio_throttle.Throttler(rate_limit=self.settings.crawler.max_concurrent)
        ...
        self.logger.info(f"异步批量爬取完成: 成功 {len([r for r in processed_results if not r.error])}, 失败 {len([r for r in processed_results if r.error])}")
    ...
    def parse_data(self, data: str, url: str) -> Any:
        """
        解析响应数据
