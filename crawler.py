...
class BaseCrawler(ABC):
    ...
    def __init__(self, name: str = "crawler"):
        ...
        self.logger = CrawlerLogger(name)
    ...
    def handle_error(self, error: Exception, url: str, attempt: int = 1) -> bool:
        ...
        Returns:
            是否应该重试
        """
        self.logger.crawl_error(url, error, attempt=attempt)
        self.stats.failed += 1
        
        if attempt < self.settings.crawler.retries:
            self.logger.retry(url, attempt + 1)
    ...
    def save_data(self, data: Any, filename: Optional[str] = None) -> str:
        ...
        Returns:
            保存的文件路径
        """
        from utils import save_data, generate_filename
